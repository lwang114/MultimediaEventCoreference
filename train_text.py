...
from corpus_text import TextEventDataset
...
def train(text_model, 
  ...
  for epoch in range(args.start_epoch, config.epochs):
    ...
    for i, batch in enumerate(train_loader):
      ...
      for idx in range(B):
        ...
        n_pairs = len(first_text_idx)
        pair_idx = 0
        text_score = []
        chunk_size = 100 # Split the pair indices to chunks to save memory
        while pair_idx < n_pairs: 
          text_score.append(text_coref_model(
                              mention_output[idx, first_text_idx[pair_idx:pair_idx+chunk_size]],
                              mention_output[idx, second_text_idx[pair_idx:pair_idx+chunk_size]]
                              )
                           )
          pair_idx += chunk_size
        text_score = torch.cat(text_score)
  ...
  if args.evaluate_only:
    results = test(text_model, mention_model, text_coref_model, test_loader, args)
  return results
...
if __name__ == '__main__':
  ...
  train_set = TextEventDataset(config, 
                               event_stoi, 
                               feature_stoi,
                               split='train')
  test_set = TextEventDataset(config, 
                              event_stoi, 
                              feature_stoi,
                              split='test')
