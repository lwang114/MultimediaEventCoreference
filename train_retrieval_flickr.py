...
from corpus_flickr_glove import FlickrGroundingGloveFeatureDataset
...
if __name__ == '__main__':
  ...
  parser.add_argument('--config', type=str, default='configs/config_grounded_mml_flickr.json')
  ...
  train_set = FlickrGroundingGloveFeatureDataset(os.path.join(config['data_folder'], 'flickr.json'), config, split='train')
  test_set = FlickrGroundingGloveFeatureDataset(os.path.join(config['data_folder'], 'flickr.json'), config, split='test')
  ...
  image_model = nn.Linear(2048, 2*embedding_dim)
