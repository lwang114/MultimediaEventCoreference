...
def load_event_features(config, split, action_labels=None):
  ...
  if config.debug:
    event_mentions = event_mentions[:20]
...
def load_visual_features(config, split):
  ...
  action_feat_type = config.get('action_feature_type', 'mmaction_event')
  if action_feat_type == 'mmaction':
    action_npz = np.load(os.path.join(config['data_folder'], f'{split}_mmaction_feat.npz'))
  elif action_feat_type == 'mmaction_event':
    action_npz = np.load(os.path.join(config['data_folder'], f'{split}_mmaction_event_feat.npz'))
  else:
    raise ValueError(f'Unknown action feature type {action_feat_type}')
  # f'{split}_mmaction_feat.npz')) # f'{split}_original_vid_3d.npz')) # f'{split}_mmaction_feat.npz')) # XXX f'{split}_events_event_type_labels.npz'
