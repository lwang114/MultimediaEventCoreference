...
class FullyContinuousMixtureAligner(object):
  ...
  def __init__(self, source_features_train, target_features_train, configs):
    self.Ks = configs.get('Ks', 33)
    self.Kt = configs.get('Kt', 200)
  ...
  def align_sents(self, source_feats_test, 
    ...
    for src_feat, trg_feat in zip(source_feats_test, target_feats_test):
      ...
      trg_null_prob = 1./self.Kt * np.ones((1, self.Kt))
      V_trg = np.concatenate([trg_null_prob, V_trg], axis=0)
       
      P_a = V_trg @ self.P_ts @ V_src.T
      scores.append(np.prod(np.max(P_a, axis=0)))
      alignments.append(np.argmax(P_a, axis=0)) 

      P_a_norm.append(P_a / P_a.sum(axis=0))
  ...
  def align(self, 
    ...
    trg_vocab['###NULL###'] = len(trg_vocab)
    n_trg_vocab += 1
    confusion = np.zeros((n_trg_vocab, n_src_vocab))
    ...
    for src_label, trg_label, alignment in zip(source_labels,
      ...
      for src_idx, y in enumerate(src_label):
        if alignment[src_idx] == 0:
          y_pred = '###NULL###'
        else:
          y_pred = trg_label[alignment[src_idx]-1]
        confusion[trg_vocab[y_pred], src_vocab[y]] += 1
    ...
    if len(source_labels) > 0:
      ...
      for i in range(n_trg_vocab):
        for j in range(n_src_vocab):
          plt.text(j, i, confusion[i, j], color='orange')
      ax.set_xticks(si[1:]-0.5)
      ax.set_yticks(ti[1:]-0.5)
      ax.set_xticklabels(sorted(src_vocab, key=lambda x:src_vocab[x]))
      ax.set_yticklabels(sorted(trg_vocab, key=lambda x:trg_vocab[x]))
  ...
  def predict_antecedent(self, P_a):
    P_a = P_a.T
    span_num = P_a.shape[0]
    P_a = np.concatenate([1./P_a.shape[1]*np.ones((1, P_a.shape[1])), P_a])
  ...
  for split in config['splits']:
    for dataset in config['splits'][split]:
      ...
      if text_feat_type == 'token':
        ...
        text_feat_files[dataset] = os.path.join(config['data_folder'], f'{dataset}_events_glove_embeddings.npz')
      elif text_feat_type == 'glove_roberta-large_pca300dim':
        text_feat_files[dataset] = os.path.join(config['data_folder'], f'{dataset}_events_glove_roberta-large_pca300dim.npz')
      else:
        raise ValueError

      visual_feat_type = config.get('visual_feature_type', 'frame')
      if visual_feat_type == 'event_average':
        visual_feat_files[dataset] = os.path.join(config['data_folder'], f'{dataset}_mmaction_event_feat_average_cluster_probs.npz') # XXX 
        visual_label_file = os.path.join(config['data_folder'], f'{dataset}_mmaction_event_feat_labels_average.npz')
        if os.path.exists(visual_label_file):
          visual_label_files[dataset] = visual_label_file
      elif visual_feat_type == 'event_concat':
        visual_feat_files[dataset] = os.path.join(config['data_folder'], f'{dataset}_mmaction_event_feat_concat_cluster_probs.npz') # XXX 
        visual_label_file = os.path.join(config['data_folder'], f'{dataset}_mmaction_event_feat_labels_concat.npz')
...
if __name__ == '__main__':
  ...
  trg_feats_train, src_feats_train,\
  trg_labels_train, src_labels_train,\
  trg_feats_test, src_feats_test,\
  trg_labels_test, src_labels_test,\
  doc_ids_train, doc_ids_test,\
  spans_train, spans_test,\
  cluster_ids_train, cluster_ids_test,\
  tokens_train, tokens_test,\
  ...
  P_a_npz = dict()
  for doc_idx, doc_id in enumerate(sorted(doc_ids_test)):
    P = P_a_norm[doc_idx]
    P_a_npz[f'{doc_id}_{doc_idx}'] = P.T
  np.savez(os.path.join(config['model_path'], 'image_to_text_normalized_align_probs.npz'), **P_a_npz) 

  antecedents = [aligner.predict_antecedent(P_a) for P_a in P_a_norm] 

  pred_labels, first, second = zip(*[aligner.predict_pairwise(P_a) for P_a in P_a_norm if P_a.shape[1] > 1])
  first = list(first)
  second = list(second)
