...
class GroundingGloveCharFeatureDataset(Dataset):
  def __init__(self, doc_json, mention_json, config, split='train'):
    ...
    self.char2id = json.load(codecs.open('data/char2id.json', 'r', 'utf-8'))
  ...
  def load_text(self, idx):
    ...
    # Extract character idxs
    doc_chars = ' '.join([t[2] for t in self.origin_tokens])
    doc_char_idxs = [self.char2id[c] for c in doc_chars] 
    doc_char_idxs = torch.LongTensor(doc_char_idxs)
    ...
    doc_char_idxs = fix_embedding_length(doc_char_idxs.unsqueeze(1), self.max_char_num).squeeze(1)
    ...
    return doc_embeddings, start_mappings, end_mappings, continuous_mappings, width, doc_char_idxs, labels, text_mask, span_mask
