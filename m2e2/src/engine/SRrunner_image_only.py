...
class SRRunner(object):
    def __init__(self, data_dir="/ws/ifp-53_2/hasegawa/lwang114/fall2020/MultimediaEventCoreference/m2e2/data/"):
        parser = argparse.ArgumentParser(description="neural networks trainer")
        parser.add_argument("--test_sr", help="situation recognition validation set", default=os.path.join(data_dir, 'imSitu/test.json'))
        parser.add_argument("--train_sr", help="situation recognition training set", default=os.path.join(data_dir, 'imSitu/train.json'))
        parser.add_argument("--dev_sr", help="situation recognition development set", default=os.path.join(data_dir, 'imSitu/dev.json')) 
        parser.add_argument("--train_ee", help="ace training set to load the vocab", required=False)
        parser.add_argument("--webd", help="word embedding", default=os.path.join(data_dir, 'glove/glove.840B.300d.txt'))
        parser.add_argument("--wnebd", help="situation recognition noun word embedding", default=os.path.join(data_dir, 'vocab/embedding_situation_noun.npy'))
        parser.add_argument("--wvebd", help="situation recognition verb word embedding", default=os.path.join(data_dir, 'vocab/embedding_situation_verb.npy'))
        parser.add_argument("--wrebd", help="situation recognition role word embedding", default=os.path.join(data_dir, 'vocab/embedding_situation_role.npy'))
        parser.add_argument("--vocab", help="situation recognition vocab_dir", default=os.path.join(data_dir, 'vocab/'))
        parser.add_argument("--image_dir", help="situation recognition image_dir", default=os.path.join(data_dir, 'imSitu/of500_images_resized'))
        parser.add_argument("--imsitu_ontology_file", help="imsitu_ontology_file", default=os.path.join(data_dir, 'imSitu/imsitu_space.json'))
        ...
        parser.add_argument("--object_detection_pkl_file", help="object_detection_pkl_file", default=os.path.join(data_dir, 'imSitu/object_detection/det_results_imsitu_oi_1.pkl'))
        ...
        parser.add_argument("--hps", help="model hyperparams", default="{'wemb_dim': 300, 'wemb_ft': False, 'wemb_ft': False, 'wemb_dp': 0.0, 'iemb_backbone': 'vgg16', 'iemb_dim': 4096, 'iemb_ft': False, 'iemb_dp': 0.0, 'posemb_dim': 512, 'fmap_dim': 512, 'fmap_size': 7, 'att_dim': 1024, 'loss_weight_verb': 1.0, 'loss_weight_verb': 1.0, 'loss_weight_noun': 0.1, 'loss_weight_role': 0.0, 'gcn_layers': 1, 'gcn_dp': False, 'gcn_use_bn': False, 'use_highway': False}")
    ...
    def load_ee_event_role(self, verb_mapping_file):
        ee_events = []
        ee_event_roles = [{}]
        for line in codecs.open(verb_mapping_file, 'r', encoding='utf-8'):
            ...
            ee_event = tabs[2]
            ee_role = tabs[3]
            if not ee_event in ee_events:
                ee_events.append(['B-' + event_type_norm(ee_event)])
            
            if not (0, 0, ee_event) in ee_event_roles[0]:
                ee_event_roles[0][(0, 0, ee_event)] = [(0, 0, role_name_norm(ee_role))]
            else:
                ee_event_roles[0][(0, 0, ee_event)].append((0, 0, role_name_norm(ee_role)))

        ee_events.append([consts.O_LABEL_NAME])
        return ee_events, ee_event_roles
    ...
    def run(self):
        ...
        pretrained_embedding = Vectors(self.a.webd, ".", unk_init=partial(torch.nn.init.uniform_, a=-0.15, b=0.15), max_vectors=10) # XXX

        ee_events, ee_event_roles = self.load_ee_event_role(self.a.verb_mapping_file)
        LabelField.build_vocab(ee_events, vectors=pretrained_embedding)
        EventsField.build_vocab(ee_event_roles, vectors=pretrained_embedding)
