...
def test(text_model,
    ...
    with torch.no_grad():     
        for i, batch in enumerate(test_loader):
          ...
          event_linguistic_labels = torch.stack(
                                [batch['linguistic_labels'][feat_type].to(device)\
                                 for feat_type in config.linguistic_feature_types], dim=2)
          arg_linguistic_labels = torch.stack(
                                [batch['arg_linguistic_labels'][feat_type].to(device)\
                                 for feat_type in config.linguistic_feature_types], dim=2) # TODO Check dim
          action_labels = batch['visual_labels'].to(device)
          text_mask = batch['text_mask'].to(device)
          span_mask = batch['span_mask'].to(device)
          action_mask = batch['visual_mask'].to(device)
          ...
          for idx in range(B):
            ...
            event_names = [event_types[y] for y in event_labels[idx, :span_num[idx]].detach().cpu().numpy()]
...
if __name__ == '__main__':
  ...
  for seed in seeds:
      ...
      # BiLSTM(int(text_coref_model.input_layer // 3),
      #        int(config.hidden_layer // 2))
      visual_model = BiLSTMVideoEncoder(
                       400, int(config.hidden_layer // 2), 
                       num_layers=3)
