...
class SMTGroundedCoreferencer:
  def __init__(self, doc_path, mention_path, embed_path, image_path, out_path, config):
    '''
    ...
    self.tokens_ids,\
    self.cluster_ids,\
    self.vocabs = self.load_corpus(doc_path, mention_path, embed_path, image_path)
  ...
  def load_corpus(self, doc_path, mention_path, embed_path, image_path):
    ...
    img_feat_keys_all = sorted(image_embeddings, key=lambda x:int(x.split('_')[-1]))
    img_feat_keys = []
    for doc_id in doc_ids:
      for feat_key in img_feat_keys_all:
        if '_'.join(feat_key.split('_')[:-1]) == doc_id:
          img_feat_keys.append(feat_key)
    image_embeddings = [image_embeddings[k] for k in img_feat_keys]
    ...
    for doc_id, feat_id in zip(doc_ids, feat_keys):
      ...
      for span in sorted(mention_dict[doc_id])[:self.max_num_mentions]:
        if span[1] >= token_embeddings[feat_id].shape[0]:
  ...
  def fit(self, n_epochs=10):
    ...
    for epoch in range(n_epochs):
      N_c = [np.zeros((len(img_feat), len(sent), len(sent))) for img_feat, sent in zip(self.image_embeddings, self.mentions)]
      N_c_all = self.alpha0 * np.ones((self.max_num_mentions, self.max_num_mentions))
      N_mc = [np.zeros((len(img_feat), len(sent), len(sent), self.max_mention_length, self.max_mention_length)) for img_feat, sent in zip(self.image_embeddings, self.mentions)]
      ...
      for sent_idx, (sent, postag, embedding, image_embedding) in enumerate(zip(self.mentions, self.postags, self.embeddings, self.image_embeddings)):
        ...
        N_c_all[:len(sent), :len(sent)] += N_c[sent_idx].sum(axis=0)
        ...
        for second_idx in range(min(len(sent), self.max_num_mentions)):
          for first_idx in range(second_idx):
            ...
            N_l[len(sent[first_idx])-1][len(sent[second_idx])-1] += N_c[sent_idx][:, first_idx, second_idx].sum()
      ...
      if pairwise_f1 > best_f1:
        ...
        with open(os.path.join(self.out_path, '../translate_probs_top200.txt'), 'w') as f:
          for img_concept in range(self.num_image_concepts):
            top_idxs = np.argsort(-N_m[img_concept].sum(axis=-1))[:200]
            for v_idx in top_idxs:
              v = vocabs[v_idx]
              p = self.mention_probs[img_concept, v_idx] 
              top_coref_idxs = np.argsort(-p)[:100]
              for c_idx in top_coref_idxs:
                f.write('Concept {}, {} -> {}: {}\n'.format(img_concept, v, vocabs[c_idx], p[c_idx]))
              f.write('\n')
  ...
  def predict(self, mentions, spans, postags, embeddings, image_embeddings):
    ...
    for m_idx, (mention, span, postag, embed) in enumerate(zip(mentions, spans, postags, embeddings)):
      ...
      second_tag = postag
      second_emb = embed
      scores = np.asarray([self.compute_mention_pair_prob(first, second, first_tag, second_tag, image_embeddings).mean()\
                           * np.exp(self.similarity(first_emb, second_emb))
...
if __name__ == '__main__':
  ...
  image_path = 'data/{}/mentions/train_mmaction_event_feat_labels.npz'.format(dataset)
  ...
  model = SMTGroundedCoreferencer(doc_path_train, mention_path_train, embed_path_train, image_path, out_path=out_path+'_train', config={'is_one_indexed': dataset == 'ecb'})
  model.fit(4)
  model.evaluate(doc_path_test, mention_path_test, embed_path_test, image_path, out_path=out_path+'_test')
