...
class CoNLLEvaluation:
  ...
  def make_output_readable(self, pred_clusters, gold_clusters, tokens, arguments=None):
    if arguments:
      pred_clusters_str = ','.join(['[{}]'.format(','.join([' '.join(tokens[m[0]:m[1]+1]) for m in cluster])) for cluster in pred_clusters])
      gold_clusters_str = ','.join(['[{}]'.format(','.join(['({}, ({}))'.format(' '.join(tokens[m[0]:m[1]+1]), ','.join([' '.join(tokens[a[0]:a[1]+1]) for a in arguments.get(tuple(m), [])])) for m in cluster])) for cluster in gold_clusters])
    else:
      pred_clusters_str = ','.join(['[{}]'.format(','.join([' '.join(tokens[m[0]:m[1]+1]) for m in cluster])) for cluster in pred_clusters])
      gold_clusters_str = ','.join(['[{}]'.format(','.join([' '.join(tokens[m[0]:m[1]+1]) for m in cluster])) for cluster in gold_clusters])
