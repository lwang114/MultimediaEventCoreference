...
class Corpus:
    ...
    def tokenize_topic(self, topic, tokenizer):
        ...
        for doc_id in self.docs_by_topic[topic]: # XXX 
            ...
            for start, end in zip(segments, segments[1:]):
                ...
                # XXX tokenizer.encode(' '.join([x[2] for x in original_tokens[original_start:original_end + 1]]),
                #                  add_special_tokens=True)[1:-1]

                if len(bert_ids) != (end - start):
                    print('bert_ids: ', bert_ids, len(bert_ids))
                    print('bert token ids: ', bert_tokens_ids, len(bert_tokens_ids))
                    print('len(origin_token), original_start, original_end, start, end, dur: ', len(original_tokens), original_start, original_end, start, end, end-start)
