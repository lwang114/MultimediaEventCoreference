    ...
    def tokenize_topic(self, topic, tokenizer):
        ...
        for doc_id in self.docs_by_topic[topic]:
            ...
            for i, token in enumerate(tokens):
                ...
                bert_token = [tokenizer.vocab[i] for i in tokenizer.preprocess(token_text)]
            ...
            segments = self.split_doc_into_segments(bert_tokens_ids, bert_sentence_ids, False)
            ...
            for start, end in zip(segments, segments[1:]):
                ...
                bert_ids = [tokenizer.vocab[i] for i in tokenizer.preprocess(' '.join([x[2] for x in original_tokens[original_start:original_end + 1]]))]
