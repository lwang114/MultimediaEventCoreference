...
class TopicSpans:
    def __init__(self, config, data, topic_num, docs_embeddings=None, docs_lengths=None, is_training=True, batch_idxs=None):
        self.config = config
        self.data = data
        ...
        self.get_all_spans_from_topic(data, topic_num, docs_embeddings, docs_lengths, batch_idxs)
        self.create_tensor()
    ...
    def get_docs_candidate(self, original_tokens, bert_start_end):
        ...
        try:
            ...
            sentences = torch.tensor([int(x[0].split('_')[-1]) for x in original_tokens])
        ...
        if bert_candidate_starts.ndim == 0:
            print('Single candidate example')
            bert_candidate_starts = bert_candidate_starts[np.newaxis]
            bert_candidate_ends = bert_candidate_ends[np.newaxis]
    ...
    def get_all_spans_from_topic(self, data, topic_num, docs_embeddings, docs_length, batch_idxs=None):
        ...
        if batch_idxs is None:
            batch_idxs = range(len(doc_names))

        for i, global_idx in enumerate(batch_idxs): # XXX
            doc_id = doc_names[global_idx]
            original_tokens = data.topics_origin_tokens[topic_num][global_idx]
            bert_start_end = data.topics_start_end_bert[topic_num][global_idx]
            ...
            self.segment_ids.extend([global_idx] * len(sentence_span))
