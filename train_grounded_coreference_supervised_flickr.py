...
from corpus_supervised_flickr_glove import FlickrSupervisedGroundingGloveFeatureDataset
    ...
    with torch.no_grad(): 
      ...
      for i, batch in enumerate(test_loader):
        ...
        for idx in range(B):
          ...
          # TODO origin_tokens = [token[2] for token in test_loader.dataset.origin_tokens[global_idx]]
...
def test_retrieve(text_model, image_model, coref_model, test_loader, args):
  ...
  with torch.no_grad():
    ...
    for i, batch in enumerate(test_loader):
      start_end_embeddings, continuous_embeddings,\
      width, videos, text_labels, img_labels, span_mask, video_mask = batch
...
if __name__ == '__main__':
  ...
  train_set = FlickrSupervisedGroundingGloveFeatureDataset(config['token_file'], config['mention_file'], config['bbox_file'], config, split='train')
  test_set = FlickrSupervisedGroundingGloveFeatureDataset(config['token_file'], config['mention_file'], config['bbox_file'], config, split='test')
