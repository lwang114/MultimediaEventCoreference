  ...
  def is_match(self, e1, e2, mode):
    ...
    if mode == MODE_S:
      ...
      if e1['is_visual'] != e2['is_visual']:
        return False
      if cosine_similarity(v1, v2) <= 0.4:
        return False 
...
def load_event_features(config, split, action_labels=None):
  ...
  event_docs_embs = np.load(os.path.join(config['data_folder'], f'{split}_events_glove_embeddings.npz')) 
  ...
  for m in entity_mentions:
    if not m['doc_id'] in entity_label_dicts:
      # if len(entity_label_dicts) >= 2: # XXX
      #   break 
      entity_label_dicts[m['doc_id']] = dict()
    m['sentence_id'] = int(m['sentence_id'].split('-')[-1])
    m = get_number_and_mention_type(m)
    m['span'] = (min(m['tokens_ids']), max(m['tokens_ids']))
    entity_label_dicts[m['doc_id']][m['m_id']] = deepcopy(m)
    
  new_entity_label_dicts = dict()
  for doc_id in entity_label_dicts:
    new_entity_dict = dict()
    for m_id, m in entity_label_dicts[doc_id].items():
      if m['mention_type'] in [PRON, NOMINAL]:
        new_entity_dict[m_id] = get_proper_mention(m, entity_label_dicts[doc_id])
      else:
        new_entity_dict[m_id] = deepcopy(m)
  ...
  for doc_id in sorted(entity_label_dicts):
    for span_idx, m_id in enumerate(sorted(entity_label_dicts[doc_id], key=lambda x:entity_label_dicts[doc_id][x]['span'])):
      entity_doc_embs = entity_docs_embs[doc_to_entity_feat[doc_id]]
      entity_label_dicts[doc_id][m_id]['entity_embedding'] = entity_doc_embs[span_idx, :300]  
  ...
  for feat_idx, doc_id in enumerate(sorted(event_label_dicts)): # XXX
    ...
    for span_idx, span in enumerate(spans):
      ...
      for a_idx, a in enumerate(event['arguments']):
        a_id = a['entity_id']
        event['arguments'][a_idx] = deepcopy(entity_label_dicts[doc_id][a_id]) 
...
def load_data(config):
  ...
  tokens_train = [] 
  ontology_map = json.load(open(os.path.join(config['data_folder'], '../ontology_mapping.json')))
  event_stoi = {k:i for i, k in enumerate(ontology_map)} # TODO
  n_types = len(event_stoi)

  for split in config['splits']['train']:
    cur_action_labels_train = None
    ...
    cur_action_feats_train = [[np.random.normal(size=(n_types,)) for e in e_feat if e['is_visual']] for e_feat in cur_event_feats_train] 
  ...
  for split in config['splits']['test']:
    ...
    cur_action_feats_test = [[np.random.normal(size=(n_types,)) for e in e_feat if e['is_visual']] for e_feat in cur_event_feats_test] # XXX
    cur_action_feats_test = [np.stack(a_feat) if len(a_feat) > 0 else np.zeros((1,n_types)) for a_feat in cur_action_feats_test]
...
if __name__ == '__main__':
  ...
  parser.add_argument('--config', '-c', default='../configs/config_amodal_smt_video_m2e2_ace.json')
